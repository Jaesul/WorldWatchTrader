...
export default function NewListingPage() {
  ...
  const isValid = price.trim() !== '' && model.trim() !== '' && description.trim() !== '';
    ...
    return (
      <div className="flex flex-col items-center justify-center px-6 py-24 text-center">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" onClick={() => { setSubmitted(false); setPhotos([]); setPrice(''); setModel(''); setDescription(''); setCondition(''); setBoxPapers(''); }}>
            Post another
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-lg px-4 pb-10 pt-6">
      <div className="mb-6">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">At least one photo, a price, a model name, and a description are required.</p>
      </div>
      ...
      <div className="space-y-5">
        <section>
          ...
          <div className="flex flex-wrap gap-2">
            {photos.map((_, i) => (
              <div key={i} className="relative flex size-20 items-center justify-center rounded-xl border border-border/60 bg-muted text-2xl">
                ⌚
                <button
                  onClick={() => setPhotos((prev) => prev.filter((__, j) => j !== i))}
          ...
          {photos.length === 0 && (
            <p className="mt-1.5 text-[11px] text-muted-foreground">Tap + to add a photo (placeholder in sandbox)</p>
          )}
        </section>

        <section>
          <label htmlFor="model" className="mb-1.5 block text-sm font-semibold text-foreground">
        ...
        <div>
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className={`size-4 transition-transform ${showOptional ? 'rotate-180' : ''}`}>
