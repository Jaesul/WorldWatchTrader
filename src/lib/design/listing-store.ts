...
const SEED_LISTINGS: MyListing[] = [
  {
    ...
    price: 12500,
    currency: "USD",
    description: "Full set, unworn. Box and papers from 2022.",
    condition: "Unworn",
    boxPapers: "Full set",
    photoCount: 3,
    photo: PHOTO_POOL[0],
  ...
  {
    id: "seed-2",
    model: "Tudor Black Bay 58 Navy",
    price: 3200,
    currency: "USD",
    description: "Great everyday diver, light wear on bracelet.",
    condition: "Good",
    boxPapers: "None",
    photoCount: 2,
    photo: PHOTO_POOL[9],
  ...
  },
  {
    ...
    price: 4100,
    currency: "USD",
    description: "Sold to happy buyer. Full set.",
    condition: "Excellent",
    boxPapers: "Full set",
    photoCount: 2,
    photo: PHOTO_POOL[4],
  ...
  try {
    ...
    if (raw) {
      ...
      listings = parsed.map((l, i) => ({
        ...
        status:
          (l.status as ListingStatus | undefined) ??
          (l.active === false ? "sold" : "active"),
      })) as MyListing[];
    } else {
...
export function addListing(
  data: Omit<MyListing, "id" | "postedAt" | "status" | "photo"> & {
    /** First listing image (e.g. data URL from upload). Falls back to mock pool when omitted. */
    photo?: string;
  },
): MyListing {
  hydrate();
  const { photo: heroPhoto, ...rest } = data;
  const newListing: MyListing = {
    ...rest,
    id: `user-${Date.now()}`,
    photo: heroPhoto ?? nextPhoto(),
    status: "active",
    postedAt: "just now",
  };
