...
export type CreateDesignListingPayload = {
  title: string;
  details: string;
  priceUsd: number;
...
export type CreateDesignListingResult =
  | { ok: true; listingId: string }
  | { ok: false; error: string };

/** Creates and publishes a listing for the `/design` cookie viewer (no World App auth). */
export async function createDesignListingAction(
  payload: CreateDesignListingPayload,
): Promise<CreateDesignListingResult> {
  const sellerId = await resolveDesignViewerUserId();
  if (!sellerId) {
    return {
      ok: false,
      error: 'No design viewer user. Add users to the database or open /design once to set a viewer.',
    };
  }
